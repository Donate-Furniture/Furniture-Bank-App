// Listing Details Page: Displays full information for a specific item, including image gallery, automated valuation, and donor contact options.
// Features a dynamic layout that adapts based on ownership (Edit/Delete vs Contact) and includes a reporting mechanism.
...
const getStatusBadge = (status: string) => {
  switch (status) {
    ...
    default:
      return (
        <span className="text-sm font-bold px-3 py-1 rounded-full bg-gray-100 text-gray-800">
          {status}
        </span>
      );
  }
...
function ListingContent() {
  ...
  // 2. Chat Auto-Open (via URL params)
  useEffect(() => {
  ...
  if (error || !listing)
    return (
      <div className="p-10 text-center text-red-500">Error loading listing</div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 lg:p-8 bg-white shadow-lg rounded-xl my-10">
      ...
      <div className="md:flex md:space-x-8">
        {/* Left Column: Images or Chat */}
        <div className="md:w-1/2 space-y-6">
          {showChat && chatRecipientId ? (
            <div>
              <button
                onClick={() => setShowChat(false)}
                className="mb-2 text-sm text-gray-500 hover:text-indigo-600 flex items-center gap-2 font-medium"
              >
                ...
                <div className="space-y-2 py-2 border-y border-gray-100">
                  <p className="text-sm flex justify-between">
                    ...
                    <span className="font-semibold">
                      {listing.city}, {listing.zipCode || "BC"}
                    </span>
                  </p>
                </div>
                ...
                {canDelete ? (
                  <div className="grid grid-cols-1 gap-3">
                    {canEdit && (
                      <Link
                        ...
                        className="flex items-center justify-center gap-2 w-full bg-indigo-600 text-white py-2.5 rounded-lg font-bold"
                      >
                    ...
                    {isOwner &&
                      (chatRecipientId ? (
                        <button
                          ...
                          className="w-full flex items-center justify-center gap-2 bg-white border border-indigo-600 text-indigo-600 py-2.5 rounded-lg hover:bg-indigo-50 font-bold"
                        >
                        ...
                        <Link
                          href="/messages"
                          className="w-full flex items-center justify-center gap-2 bg-white border border-gray-300 text-gray-700 py-2.5 rounded-lg hover:bg-gray-50 font-bold"
                        >
                          Check Inbox
                        </Link>
                      ))}
                    ...
                    <button
                      ...
                      disabled={isDeleting}
                      className="flex items-center justify-center gap-2 w-full bg-white border border-red-200 text-red-600 py-2.5 rounded-lg hover:bg-red-50 font-bold"
                    >
                  ...
                  <div className="space-y-3">
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 bg-indigo-600 text-white py-3 rounded-xl font-bold shadow-md"
                    >
                    ...
                    {user && (
                      <button
                        ...
                        className="w-full text-center text-xs text-gray-400 hover:text-red-500 hover:underline mt-2 transition-colors"
                      >
                        Report this listing
                      </button>
                    )}
        ...
        <div className="md:w-1/2 p-6 md:p-10 border-l border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            ...
            <div className="col-span-2 p-3 bg-red-50 border border-red-100 rounded-lg flex gap-3 items-center">
              <Clock />
              <div>
                <p className="text-xs text-red-500 font-bold">Deadline</p>
          ...
          <p className="text-xs text-gray-400 italic mt-8 border-t pt-2">
            * Automated estimate based on standard depreciation.
          </p>
        </div>
      </div>
...
// Suspense Wrapper for Safe SearchParams usage
