// Auth Page: Unified entry point for Authentication (Login & Registration).
// Handles Credential auth (Email/Pass), Social auth (Google/FB/Apple), and toggles between "Sign In" and "Sign Up" modes.
...
export default function AuthPage() {
  ...
  // Default to Register mode if query param ?mode=register is present
  const initialMode = searchParams.get("mode") === "register" ? false : true;

  const [isLogin, setIsLogin] = useState(initialMode);

  // Unified form state for both Login and Register
      ...
      <div className="w-full max-w-md space-y-8 bg-white p-8 rounded-xl shadow-lg border border-gray-100">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            {isLogin
              ...
              : "Join the community to trade furniture"}
          </p>
        </div>
        ...
        <form className="mt-6 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="rounded-md shadow-sm space-y-4">
            {/* --- REGISTER FIELDS --- */}
            {/* These fields only show up when mode is 'Register' */}
            {!isLogin && (
              <>
